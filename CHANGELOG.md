...
### Changed
- 跟随视频播放进度滚动按钮样式优化，且在视频暂停时也显示
- 当字幕扩大搜索和普通搜索的结果一模一样时，不显示扩大搜索
- “AI修正字幕”不再为AI提供时间戳，大幅提高速度
- 展开字幕区折叠栏时，自动请求所有多语言字幕（文件较小，但可以在网络环境不佳时避免长时间等待字幕请求）
- “复制全部”按钮改为白底，锁定为不包含时间戳，可以在文本页复制包含时间戳的

### Fixed
- 面板拖动扩展区和指示条不能适配当按钮不处于默认右上角的问题
- “导出SRT”按钮悬停时缺少动画
- 编辑字幕页，点击导出SRT后，toast层级错误
- 播放按钮的弹窗动画问题
- 下载按钮的弹窗，当切换TXT和SRT时，弹窗高度瞬变的问题（改为动画）
...
### Changed
- 引导文本优化
- 面板宽度除手动拖动外，还恢复此前的自动根据浏览器窗口大小调整，两个机制共同作用
- 播放模式中，播放按钮变蓝
- 下载图标优化
- 跟随视频滚动字幕功能，只有当视频正在播放时才出现按钮

### Fixed
- 播放模式弹窗中示例字幕位置和实际字幕位置偏移的问题
- 关闭播放和下载弹窗时把主面板也关闭的问题
