...
(function () {
    ...
    let DEEPSEEK_API_KEY = GM_getValue('bse_api_key', '');
    let DEEPSEEK_API_URL = GM_getValue('bse_api_url', 'https://api.deepseek.com/v1/chat/completions');
...
使用清晰的Markdown格式进行排版，包括正确的分级标题、列表缩进等。`;
    ...
    async function fetchBilibiliSubtitles() {
        ...
        try {
            ...
            return pd.data.subtitle.subtitles
        ...
        } catch(e) { log('B站API错误:', e); return []; }
    }

    async function fetchSubtitleContent(url) {
        try {
            if (url.startsWith('//')) url = 'https:'+url;
            const r = await fetch(url); const d = await r.json(); return d.body||[];
        } catch { return []; }
    }
    ...
    async function callDeepSeekAPIStream(messages, onChunk) {
        const resp = await fetch(DEEPSEEK_API_URL, {
            method:'POST',
            headers:{'Content-Type':'application/json','Authorization':`Bearer ${DEEPSEEK_API_KEY}`,'Accept':'text/event-stream'},
            body:JSON.stringify({model:DEEPSEEK_MODEL, messages, temperature:0.7, max_tokens:4000, stream:true})
        });
        ...
        while (true) {
            ...
            for (const line of lines) {
                ...
                if (t.startsWith('data:')) {
                    const ds = t.slice(5).trim(); if (ds==='[DONE]') return full;
                    try { const d=JSON.parse(ds); const c=d.choices?.[0]?.delta?.content; if(c){full+=c;onChunk(full);} } catch {}
                }
            }
        }
        return full;
    ...
    function callDeepSeekAPI(messages) {
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                method:'POST', url:DEEPSEEK_API_URL,
                headers:{'Content-Type':'application/json','Authorization':`Bearer ${DEEPSEEK_API_KEY}`},
                data:JSON.stringify({model:DEEPSEEK_MODEL, messages, temperature:0.7, max_tokens:200}),
                timeout:60000,
                onload(r) {
                    try {
                        const d=JSON.parse(r.responseText);
                        if (d.error) return reject(new Error(d.error.message));
                        if (d.choices?.[0]?.message?.content) return resolve(d.choices[0].message.content);
    ...
    async function generateAISummaryStream(subtitleText, streamEl) {
        ...
        let summary = await callDeepSeekAPIStream(messages, text => {
        ...
        if (adCheck.type === 'error') {
            ...
            try {
                const fix = await callDeepSeekAPI(messages);
                log('打回修正结果:', fix);
                summary = summary + '\n' + fix.trim();
    ...
    async function fetchAllSubtitles(force=false) {
        ...
        currentVideoKey=vk; allSubtitles=[]; currentSubtitleData=null; selectedSubtitleId=null; adSegments=[]; hasJumpedAds={};
        const existingMark = document.getElementById('bse-ad-progress-mark');
        if (existingMark) existingMark.remove();
    ...
    async function loadSubtitle(sub) {
        ...
        const afterLoad = () => {
            ...
            if (autoGenSummary && currentSubtitleData?.body?.length && !getCachedSummary(currentVideoKey) && DEEPSEEK_API_KEY) {
                setTimeout(() => {
                    switchTab('ai');
                    setTimeout(() => document.getElementById('bse-generate-btn')?.click(), 50);
                }, 400);
            }
        };
            ...
            <div class="bse-panel">
                ...
                <div class="bse-footer">
                    <button class="bse-btn bse-btn-secondary" id="bse-download-btn" disabled>
                        <svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>下载
                    </button>
    ...
    function bindEvents(c) {
        ...
        c.querySelector('#bse-copy-btn').addEventListener('click', () => {
            const t=getFormattedText(); if(t){GM_setClipboard(t);showToast('✓ 已复制','success');}
        });
        c.querySelector('#bse-download-btn').addEventListener('click', () => {
    ...
    function updateUI() {
        ...
        const dlBtn=document.querySelector('#bse-download-btn');
        ...
        if (sb) {
            if (allSubtitles.length>0) {
                ...
                sb.querySelectorAll('.bse-subtitle-option').forEach(o=>{
                    o.addEventListener('click',()=>{ const s=allSubtitles.find(x=>x.id==o.dataset.id); if(s) loadSubtitle(s); });
                });
            } else {
        ...
        if (currentSubtitleData?.body) {
            ...
            if(copyBtn) copyBtn.disabled=false; if(dlBtn) dlBtn.disabled=false;
        } else if (!isLoading) {
    ...
    function updateContent() {
        const el = document.querySelector('.bse-content'); if(!el) return;
        if (isLoading) { safeSetInnerHTML(el,'<div class="bse-loading"><div class="bse-spinner"></div><div>数据加载中...</div></div>'); return; }
        switch(currentTab) {
    ...
    function renderPreviewTab(el) {
        ...
        const body=currentSubtitleData.body, cnt=body.length, dur=body[cnt-1].to, chars=body.reduce((s,i)=>s+i.content.length,0);
        safeSetInnerHTML(el, `
            <div class="bse-stats">
    ...
    function renderAITab(el) {
        ...
        if (!cachedSummary) {
            html += `<button class="bse-ai-big-btn" id="bse-generate-btn" ${!hasSubtitle || !DEEPSEEK_API_KEY?'disabled':''}>
            ...
            const retryHtml = `<button class="bse-retry-btn" id="bse-retry-btn" title="重新生成"><svg viewBox="0 0 24 24"><path d="M17.65 6.35A7.958 7.958 0 0012 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08A5.99 5.99 0 0112 18c-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/></svg></button>`;
            ...
            if (showRawAIText) {
                ...
                const adData = extractAdSegments(cachedSummary);
                adSegments = adData.segments;
                log('【渲染AI页】广告状态:', adData.type, JSON.stringify(adData.segments));
                ...
                if (adData.type === 'has_ad' && adSegments.length > 0) {
                    html += `<div class="bse-sp-box status-found">
                        ...
                        <div class="bse-sp-hint">播放至该时段将自动跳过，进度条已标黄提示</div>
                    </div>`;
                ...
                html += `<div class="bse-followup-section">
                    ...
                    <textarea class="bse-followup-input" id="bse-followup-input" placeholder="就视频内容提问"></textarea>
                    <button class="bse-followup-btn" id="bse-followup-btn">发送追问</button>
                </div>`;
            }
        ...
        async function doGenerate(e) {
            ...
            if (!hasSubtitle || !DEEPSEEK_API_KEY) return;
            ...
            safeSetInnerHTML(el, `<div class="bse-ai-result bse-markdown" id="bse-stream-body" style="min-height:400px;overflow-y:auto;"><div class="bse-loading"><div class="bse-spinner"></div><div>AI分析中...</div></div></div>`);
        ...
        if (fBtn && fInput) {
            const send = async () => {
                const q=fInput.value.trim(); if(!q) return;
                ...
                try {
                    const a = await callDeepSeekAPIStream(aiConversationHistory, text=>{ safeSetInnerHTML(ansEl, markdownToHtml(text)); });
                    aiConversationHistory.push({role:'assistant',content:a});
                    appendCachedQA(currentVideoKey, q, a);
                    fInput.value=''; showToast('✓ 回复完成','success');
                } catch(e) {
    ...
    function renderTextTab(el) {
        if (!currentSubtitleData?.body?.length) { safeSetInnerHTML(el,'<div class="bse-empty">暂无数据</div>'); return; }
        safeSetInnerHTML(el, `
        ...
        safeSetInnerHTML(el, `
            <div style="padding:10px 0;">
                <div class="bse-settings-block">
                    ...
                    <input type="password" class="bse-settings-input" id="bse-s-key" value="${DEEPSEEK_API_KEY}" placeholder="sk-...">
                </div>
                <div class="bse-settings-block">
                    <label class="bse-settings-block-label">API URL Endpoint</label>
                    <input type="text" class="bse-settings-input" id="bse-s-url" value="${DEEPSEEK_API_URL}" placeholder="https://api.deepseek.com/v1/chat/completions">
                </div>
                ...
                <div style="margin-top:16px;text-align:center;">
                    <a href="https://platform.deepseek.com" target="_blank" style="font-size:12px;color:var(--bse-text-muted);text-decoration:none;">获取 DeepSeek API（性价比高） →</a>
                </div>
                <div class="bse-author-info">
                    ...
                    <p class="bse-author-text" style="margin-top:8px;">部分使用了M0M Chen的 视频字幕提取器Pro 代码（MIT）</p>
                </div>
            </div>
        `);
        document.getElementById('bse-s-cancel')?.addEventListener('click', ()=>{
            switchTab('preview');
        });
        document.getElementById('bse-s-save')?.addEventListener('click', ()=>{
            DEEPSEEK_API_KEY = document.getElementById('bse-s-key').value.trim();
            DEEPSEEK_API_URL = document.getElementById('bse-s-url').value.trim();
            autoGenSummary = document.getElementById('bse-s-auto').checked;
            GM_setValue('bse_api_key', DEEPSEEK_API_KEY);
            GM_setValue('bse_api_url', DEEPSEEK_API_URL);
    ...
    function resetState() {
        log('路由变更，重置状态');
        currentVideoKey=null; allSubtitles=[]; currentSubtitleData=null; selectedSubtitleId=null;
        aiConversationHistory=[]; adSegments=[]; hasJumpedAds={}; showRawAIText=false;
        const existingMark = document.getElementById('bse-ad-progress-mark');
        if (existingMark) existingMark.remove();
        updateUI();
