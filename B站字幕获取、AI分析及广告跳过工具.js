...
(function () {
    ...
    function getAISummaryPrompt() {
        ...
        switch (bse_detail_level) {
            case 'very_detailed':
                summaryWord = '非常详细'; overviewWord = '全面'; listWord = '详细地分点列出核心结论、关键信息和具体细节（包含论述过程和支撑论据）'; break;
            case 'detailed':
                summaryWord = '详细'; overviewWord = '详细'; listWord = '详细地分点列出核心结论和关键信息'; break;
            case 'minimal':
                summaryWord = '极简'; overviewWord = '极简'; listWord = '极简地分点列出核心要点（剔除一切修饰性废话）'; break;
            default:
                summaryWord = '简洁'; overviewWord = '简明'; listWord = '精简地分点列出核心结论和关键信息（剔除修饰性废话）'; break;
        }
...
使用清晰的Markdown格式进行排版，包括正确的分级标题、列表缩进等。`;
    ...
    GM_addStyle(`
        ...
        * { font-family: -apple-system,BlinkMacSystemFont,"Microsoft YaHei",sans-serif !important; }
        ...
        .bse-icon-btn { width:34px; height:34px; border-radius:var(--bse-radius-sm); background:var(--bse-bg-card); border:1px solid var(--bse-border); cursor:pointer; display:flex; align-items:center; justify-content:center; color:var(--bse-text-dim); transition:all 0.2s; text-decoration:none; }
        ...
        .bse-update-badge { display:inline-flex; align-items:center; gap:4px; padding:2px 8px; background:linear-gradient(135deg, #ef4444, #dc2626); color:white; font-size:11px; font-weight:700; border-radius:8px; cursor:pointer; text-decoration:none; transition:all 0.2s; margin-left:4px; vertical-align:middle; white-space:nowrap; }
        .bse-update-badge:hover { transform:scale(1.05); box-shadow:0 2px 8px rgba(220,38,38,0.4); color:white; text-decoration:none; }

        .bse-ext-links { display:flex; gap:8px; justify-content:center; align-items:center; margin-bottom:14px; }
        .bse-ext-link { display:inline-flex; align-items:center; gap:5px; padding:5px 12px; border-radius:8px; text-decoration:none; font-size:12px; font-weight:500; transition:all 0.2s; color:var(--bse-text-dim); background:var(--bse-bg-card); border:1px solid var(--bse-border); }
        .bse-ext-link:hover { color:var(--bse-text); border-color:#cbd5e1; transform:translateY(-1px); box-shadow:0 2px 6px rgba(0,0,0,0.06); text-decoration:none; }
                ...
                if (response.status === 200) {
                    ...
                    if (match && match[1]) {
                        ...
                        if (compareVersions(latestVersion, SCRIPT_VERSION) > 0) {
                            ...
                            log('当前已是最新版本:', SCRIPT_VERSION);
                        }
                    }
                }
            },
            onerror: function () { log('更新检测: 网络请求失败'); },
            ontimeout: function () { log('更新检测: 请求超时'); }
        });
    }
    ...
    async function callAPIStream(messages, onChunk) {
        ...
        if (isClaude) {
            ...
            fetchUrl = fetchUrl.replace('{model_name}', actualModel); if (fetchUrl.includes(':generateContent')) fetchUrl = fetchUrl.replace(':generateContent', ':streamGenerateContent'); fetchUrl += (fetchUrl.includes('?') ? '&' : '?') + `key=${API_KEY}&alt=sse`; bodyData = { contents: messages.map(m => ({ role: m.role === 'assistant' ? 'model' : 'user', parts: [{ text: m.content }] })) };
        } else {
            headers['Authorization'] = `Bearer ${API_KEY}`; headers['Accept'] = 'text/event-stream'; bodyData = { model: actualModel, messages: messages, stream: true };
        }
    ...
    async function generateAISummaryStream(subtitleText, streamEl) {
        ...
        setCachedSummary(currentVideoKey, summary); aiConversationHistory = [{ role: 'user', content: getAISummaryPrompt() }, { role: 'assistant', content: summary }];
    ...
    async function fetchAllSubtitles(force = false) {
        ...
        try { allSubtitles = await fetchBilibiliSubtitles(); const commentPromise = fetchHotComments(); if (allSubtitles.length > 0) await loadSubtitle(allSubtitles[0]); hotComments = await commentPromise; } catch (e) {}
    ...
    function bindEvents(c) {
        ...
        _documentClickHandler = e => { if (!panelVisible) return; if (!c.contains(e.target)) { panelVisible = false; panel.classList.remove('show'); } };
    ...
    function updateUI() {
        ...
        if (sb) { if (allSubtitles.length > 0) { safeSetInnerHTML(sb, allSubtitles.map(s => `<div class="bse-subtitle-option ${s.id === selectedSubtitleId ? 'active' : ''}" data-id="${s.id}">${s.lan_doc}<span class="bse-tag ${s.isAI ? 'ai' : 'cc'}">${s.isAI ? 'AI' : 'CC'}</span></div>`).join('')); sb.querySelectorAll('.bse-subtitle-option').forEach(o => o.addEventListener('click', (e) => { e.stopPropagation(); const s = allSubtitles.find(x => x.id == o.dataset.id); if (s) loadSubtitle(s); })); } else { safeSetInnerHTML(sb, '<div style="color:var(--bse-text-dim);font-size:13px;padding-bottom:4px;">未检测到可用的中文字幕</div>'); } }
    ...
    function renderSettingsTab(el) {
        ...
        document.getElementById('bse-s-save')?.addEventListener('click', () => {
            ...
            GM_setValue('bse_platform', bse_platform); GM_setValue('bse_api_url', API_URL); GM_setValue('bse_api_key_' + bse_platform, API_KEY); GM_setValue('bse_model', bse_model); GM_setValue('bse_auto_summary', autoGenSummary); GM_setValue('bse_opinion_analysis', enableOpinionAnalysis); GM_setValue('bse_auto_skip_ad', bse_auto_skip_ad); GM_setValue('bse_auto_open_panel', autoOpenPanel); GM_setValue('bse_auto_open_tab', autoOpenTab); GM_setValue('bse_detail_level', bse_detail_level);
            showToast('✓ 设置已保存', 'success'); switchTab('preview'); panelVisible = false; const container = document.querySelector('.bse-container'); if (container) container.remove(); createUI(); setTimeout(() => fetchAllSubtitles(true), 200);
        });
    }

    // ===================== 23. 初始化与路由监听 =====================
    function init() { log('B站字幕获取、AI分析及广告跳过工具 v' + SCRIPT_VERSION + ' 已加载。作者：LiuMashiro'); aiSummaryCache = loadCache(); createUI(); setTimeout(() => { fetchAllSubtitles(); initAdSkipMonitor(); }, 1500); setTimeout(() => { checkForUpdates(); }, 5000); }
